...
# Qwen3.8-27B static-YaRN 1M SFT.
# Default topology: 64 GPUs = 8 nodes x 8 GPUs, SP=8, DP=8, TP=EP=1.
#
# IMPORTANT: with the current XTuner Ulysses implementation, Qwen3.8-27B has
# SP_SIZE <= 8 because its 24 full-attention heads and 16/48 linear-attention
# heads share no larger divisor. At 1M/SP8, BF16 checkpoint-boundary
# activations alone are roughly 76 GiB per rank. This launcher therefore
# targets H200/B200-class GPUs (at least 141 GB). It is not an 80 GB-GPU recipe.
#
...
output_root=${OUTPUT_ROOT:-/mnt/shared-storage-gpfs2/intern-pretrain-shared02/xiexinchen/xtuner_scripts/outputs/qwen38-27b-1m_frontend-0807}
...
min_gpu_memory_mib=${MIN_GPU_MEMORY_MIB:-120000}
allow_low_memory_1m=${ALLOW_LOW_MEMORY_1M:-false}

image=${IMAGE:-registry.h.pjlab.org.cn/ailab-llmrazor/xtuner_tmp:pt28_20260303_f2adb47}
meta_name=$(basename "${meta_data_path}")
meta_name=${meta_name%.*}
run_tag=${RUN_TAG:-${meta_name}-1m-$(date +%Y%m%d-%H%M)}
job_name=${JOB_NAME:-qwen38-27b-1m-${run_tag}}
...
rjob submit \
    ...
    -e LOG_DIR="${log_dir}" \
    -e LOG_FILE="${log_file}" \
    -e MIN_GPU_MEMORY_MIB="${min_gpu_memory_mib}" \
    -e ALLOW_LOW_MEMORY_1M="${allow_low_memory_1m}" \
    ...
    -e SAMPLE_MAX_LENGTH=1000000 \
    -e PACK_MAX_LENGTH=1000000 \
    -e MAX_POSITION_EMBEDDINGS=1000000 \
    -e ROPE_SCALING_FACTOR=4.0 \
    -e GLOBAL_BATCH_SIZE=8 \
    -e SP_SIZE=8 \
    -e TP_SIZE=1 \
    -e EP_SIZE=1 \
    -e NUM_WORKERS=4 \
    -e PACK_EXTRA_BUFFER_SIZE=8 \
    ...
    -- bash -lc '
        ...
        gpu_memory_mib=$(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits | sed -n "1p" | tr -d " ")
        echo "GPU=${gpu_name}; memory=${gpu_memory_mib} MiB"
        if [ "${gpu_memory_mib}" -lt "${MIN_GPU_MEMORY_MIB}" ] && [ "${ALLOW_LOW_MEMORY_1M}" != "true" ]; then
            echo "ERROR: 1M/SP8 needs a >=${MIN_GPU_MEMORY_MIB} MiB GPU for this recipe; refusing a likely OOM."
            echo "Set ALLOW_LOW_MEMORY_1M=true only if you intentionally want to test an unsupported low-memory run."
            exit 3
        fi
        test -f "${CONFIG_FILE}"
